      ...
      for k,triple in group.iterrows():
        ...
        if not group_entities:
          ...
          if res1:
              ...
              if pd.isna(triple['triple_subj_head']):
                  if len(triple['triple_subj'].split(' ')) == 1:
                      reg = re.compile(re.escape(triple['triple_subj']))
                  elif str(triple['triple_subj']).istitle():
                      splits = triple['triple_subj'].split(' ')
                      reg = re.compile(re.escape(splits[0]) + '.*' + re.escape(splits[-1]))
                  else:
                      reg = re.compile(re.escape(triple['triple_subj'].split(' ')[-1]))
              else:
          ...
          if res2:
              ...
              if pd.isna(triple['triple_obj_head']):
                  if len(triple['triple_obj'].split(' ')) == 1:
                      reg = re.compile(re.escape(triple['triple_obj']))
                  elif str(triple['triple_obj']).istitle():
                      splits = triple['triple_obj'].split(' ')
                      reg = re.compile(re.escape(splits[0]) + '.*' + re.escape(splits[-1]))
                  else:
                      reg = re.compile(re.escape(triple['triple_obj'].split(' ')[-1]))
              else:
...
if __name__ == '__main__':
    ...
    outputFile = os.path.join(test_folder, 'triples_openalex_all_EntityNormalized_EntityLinking.tsv')
    ...
    #triples = pd.read_excel(os.path.join(test_folder, 'triples_openalex_all_EntityNormalized.tsv'), engine="openpyxl")

    triples = pd.read_csv(os.path.join(test_folder, 'triples_openalex_all_EntityNormalized.tsv'), lineterminator='\n', low_memory=False, sep='	', on_bad_lines='skip')
    triples.drop(columns='Unnamed: 0', inplace=True)
    triples.drop_duplicates(keep="first", inplace=True)
