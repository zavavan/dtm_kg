    ...
    for token in poss:
        ...
        if sentStart:
            ...
            for i in range(min(2, sent_len)):
                if poss[token.i + i].text.startswith('@') and 'VB' not in poss[token.i + i + 1].tag_:
                    found = True
                    break
            if found:
...
if __name__ == '__main__':
    ...
    CERT_FINGERPRINT = "1f69f5ef2ac58a54075f944fd13edbcdca996b43813b5c5c92bcc865cb586a26"
    # Password for the 'elastic' user generated by Elasticsearch
    ELASTIC_PASSWORD = "rMOrxmKNS32XGRWBd_4o"
    es = Elasticsearch("http://localhost:9200", request_timeout=900000)
    # Successful response!
    print(es.info())
        ...
        "query": {
            "nested": {
                ...
                "query": {
                    "bool": {
                        "must": [
                            {"match": {"hashtags.tag": "DigitalTransformation"}
                             }
    ...
    resp = es.search(index="digital_transformation_tweets", _source=['tweetid'], body=body)
    print("Got %d Hits:" % resp['hits']['total']['value'])

    for hit in resp['hits']['hits']:
        print(hit["_source"])

    file = open(os.path.join('D:/GitRepos/GitRepos/skg/data-collection/twitter', 'sample.tsv'), mode="r",
                encoding="utf-8")
                    ...
                    if len(paths) > 0:
                        ...
                        for path in paths:
                            ...
                            if result1 is not None and result2 is not None:
                                ...
                                relString = tokens[find_rel_index(path)].text

                                relStringLemmatized = tokens[find_rel_index(path)].lemma_

                                relations.append(
                                    [relString, relStringLemmatized, counter_used_docs, find_rel_index(path)])
    ...
    kg_df_corenlp.to_excel(os.path.join(test_folder,
                                        'en_core_web_lg_preprocessing_depTrees_coref.xlsx' if process_annotation == True else 'en_core_web_lg_no_preprocessing_depTrees_coref.xlsx'),
    ...
    df_accepted.to_csv(os.path.join(test_folder, 'TokenLengthDistributionTweetExtractedPath_coref.tsv'), sep=' ')
    ...
    df_discarded.to_csv(os.path.join(test_folder, 'TokenLengthDistributionTweetDiscardedPath_coref.tsv'), sep=' ')
    ...
    f1.savefig(os.path.join(test_folder, 'TokenLengthDistributionTweetExtractedPath_coref.png'))
    ...
    # ax.set_ylabel('Number of Tweets', labelpad=20, weight='bold', size=12)
    # ax.tick_params(axis="both", which="both", bottom="off", top="off", labelbottom="on", left="off", right="off", labelleft="on")
    f2.savefig(os.path.join(test_folder, 'TokenLengthDistributionTweetDiscardedPath_coref.png'))
    ...
    df_entity_lemma_distr.to_excel(os.path.join(test_folder, 'LemmatizedEntityDistribution_coref.xlsx'),
                                   encoding="utf-8", engine="xlsxwriter")
    ...
    df_relation_lemma_distr.to_excel(os.path.join(test_folder, 'LemmatizedRelationDistribution_coref.xlsx'),
                                     encoding="utf-8", engine="xlsxwriter")
    ...
    df_relations.to_csv(os.path.join(test_folder, 'Relations.csv'), encoding="utf-8")

    print('Total number of coreferences: ' + str(coreferences))

    db.to_disk(os.path.join(test_folder, 'doc_bin'))
